...
router.post(
  ...
  wrap.query(async (req, res, next, mapper) => {
    ...
    let memberIdx = req.session.member.idx;
    let mimetype = file.mimetype;
    let ext = file.mimetype.split('/')[1];
    let name = file.originalname;
    let size = file.size;
    let src = `${CONFIG.CTX.DOMAIN}/uploads/${dypth}/${file.filename}`;

    let upload = { memberIdx, mimetype, ext, name, size, src };

    const [{ insertId }] = await mapper('Upload', 'insertOne', upload);
...
router.post(
  ...
  wrap.query(async (req, res, next, mapper) => {
    ...
    await mapper('Upload', 'insertOne', upload);
...
router.post(
  '/thumbnail',
  sessionCtx.isAdmin(), // 관리자 권한
  validateCtx.query('width', 'height'),
  wrap.query(async (req, res, next, mapper) => {
    ...
    let size = file.size;
    let src = `${CONFIG.CTX.DOMAIN}/uploads/${dypth}/th_${file.filename}`;
